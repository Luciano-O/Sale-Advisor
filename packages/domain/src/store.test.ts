...
describe("normalizeStore", () => {
  it("normalizes domain casing and removes www prefix", () => {
    const store = normalizeStore({ normalizedUrl: requiredUrl("https://WWW.LOJA-A.EXAMPLE.BR/produto?sku=ABC123") });
  ...
  it("extracts store product id from sku query parameter", () => {
    const store = normalizeStore({ normalizedUrl: requiredUrl("https://loja-a.example.br/gpu/rtx-4060?sku=RTX4060-8GB") });
  ...
  it("extracts store product id from productId and produtoId query parameters", () => {
    const byProductId = normalizeStore({ normalizedUrl: requiredUrl("https://loja-b.example.br/item?productId=98765") });
    const byProdutoId = normalizeStore({ normalizedUrl: requiredUrl("https://loja-b.example.br/item?produtoId=54321") });
  ...
  it("extracts a stable numeric id from loja-c path", () => {
    const store = normalizeStore({ normalizedUrl: requiredUrl("https://loja-c.example.br/produto/rtx-4060-8gb/123456") });
  ...
  it("uses generic query extraction for unknown domains", () => {
    const store = normalizeStore({ normalizedUrl: requiredUrl("https://marketplace.example.br/oferta?id=GEN-42") });
  ...
  it("returns null store product id when no reliable id exists", () => {
    const store = normalizeStore({ normalizedUrl: requiredUrl("https://loja-a.example.br/promocoes/rtx-4060") });
